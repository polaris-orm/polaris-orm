 ...
 * "databaseProductName" taken from the {@link DatabaseMetaData}.
 *
...
public class SQLErrorCodesFactory {
   ...
   */
  public static final String SQL_ERROR_CODE_DEFAULT_PATH = "cn/taketoday/polaris/jdbc/support/sql-error-codes.xml";
   ...
   * {@link DatabaseMetaData}, or an empty error codes
   ...
   * Return {@link SQLErrorCodes} for the given {@link DataSource},
   * evaluating "databaseProductName" from the
   * {@link DatabaseMetaData}, or {@code null} if case
  ...
  /**
   * 从给定的 {@link ConnectionSource} 返回 {@link SQLErrorCodes}
   */
  @Nullable
  public SQLErrorCodes resolveErrorCodes(ConnectionSource source) {
    Assert.notNull(source, "ConnectionSource is required");
    DataSource dataSource = source.getDataSource();
    if (dataSource != null) {
      return resolveErrorCodes(dataSource);
    }

    if (log.isDebugEnabled()) {
      log.debug("Looking up default SQLErrorCodes for ConnectionSource [{}]", source);
    }

    try (Connection con = source.getConnection()) {
      String databaseName = con.getMetaData().getDatabaseProductName();
      if (StringUtils.isNotEmpty(databaseName)) {
        return getErrorCodes(databaseName);
      }
    }
    catch (SQLException ex) {
      log.warn("Error while extracting database name", ex);
