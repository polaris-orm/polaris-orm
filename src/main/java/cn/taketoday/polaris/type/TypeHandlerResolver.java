...
public interface TypeHandlerResolver {
  ...
  static TypeHandlerResolver forAnnotation(Class<? extends Annotation> annotationType, String attributeName) {
    ...
    return (BeanProperty property) -> {
      Annotation mappedTypeHandler = property.getAnnotation(annotationType);
      if (mappedTypeHandler != null) {
        // user defined TypeHandler
        var typeHandlerClass = (Class<? extends TypeHandler<?>>) AnnotationUtils.getValue(mappedTypeHandler, attributeName);
